...
@[simp] private def th1 (a b : Nat) : a < 1 + 1 + b + a := by
  have h : 1 + 1 + b + a = a + (1 + 1 + b) := by ring
  simp [h]
  apply Nat.lt_add_of_pos_right
  simp [Nat.lt_add_right]

@[simp] private def th2 (a b : Nat) : a + b < 1 + 1 + c + a + b := by
  have h : 1 + 1 + c + a + b= a + b + (1 + 1 + c) := by ring
  simp [h]
  apply Nat.lt_add_of_pos_right
  simp [Nat.lt_add_right]

@[simp] private def th3 (a b : Nat) : b + c < 1 + 1 + a + b + (1 + 1 + c + d) := by
  have h : 1 + 1 + a + b + (1 + 1 + c + d) = b + c + (1 + 3 + a + d) := by ring
  simp [h]
  apply Nat.lt_add_of_pos_right
  simp [Nat.lt_add_right]

@[simp] private def th4 (a b : Nat) : a + b < a + (1 + 1 + b + c) := by
  have h : a + (1 + 1 + b + c) = a + b + (1 + 1 + c) := by ring
  simp [h]
  apply Nat.lt_add_of_pos_right
  simp [Nat.lt_add_right]
...
termination_by _ a b => sizeOf a + sizeOf b
decreasing_by 
...
instance [Repr α] [Repr β] : Repr (RBMap α β cmp) where
  reprPrec m prec := Repr.addAppParen ("MStd.rbmapOf " ++ repr m.toList) prec

@[inline] def insert : RBMap α β cmp → α → β → RBMap α β cmp
  | ⟨t, w⟩, k, v => ⟨t.insert cmp k v, True.intro⟩

@[inline] def erase : RBMap α β cmp → α → RBMap α β cmp
  | ⟨t, w⟩, k => ⟨t.erase cmp k, True.intro⟩
